function [J,grad] = genLoss(discParams,genParams,discHiddenSize,genHiddenSize, ...
                            discInputSize,genInputSize,noise,lambda)
...
J = J*(-1);
 ...
 % adding regularisation terms
    ...
    for i=2:(genInputSize+1)
        J=J+lambda*theta1_gen(:,i)'*theta1_gen(:,i)/(2*m_fake);
    end
    
    for i=2:(genHiddenSize+1)
       J=J+lambda*theta2_gen(:,i)'*theta2_gen(:,i)/(2*m_fake);
    end
